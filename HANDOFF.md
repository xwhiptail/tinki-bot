...
- Status: Tinki can now be mentioned with a Discord message link to reply directly to that linked message when it is in the current server, pasted Discord message links and preview-surfaced mentions after a link no longer count as speaking to Tinki, and OpenAI insufficient-quota failures produce a distinct out-of-money response; GPT-5 chat completion calls use `max_completion_tokens`; Instagram URL rewrites target `vxinstagram.com`; the shell deploy helper overwrites existing repo files during recursive uploads on AL2023; new AL2023 host is live, hardened, and running Python 3.11; local macOS repo has a working `.venv` pytest setup, and the repo includes low-cost monitoring setup helpers for AWS Budgets, CloudWatch alarms, and host memory/disk metrics
- Active branch: `main`
...
- Last known good verification: `python -m pytest -q` locally (`286 passed`) after tightening Tinki mention detection around Discord message links and updating GPT-5 token-limit kwargs, plus `python -m pytest --collect-only -q`, focused AI listener/OpenAI helper/natural-command slices, a mojibake scan through the full suite, and a remote OpenAI sanity call with `max_completion_tokens`
