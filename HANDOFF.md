...
- Last known good verification: `python -m pytest -q` locally (`312 passed`) after hardening current-awareness ranking and validation for the AoE4 May 7 civ and current FFXIV expansion questions, plus `python -m pytest --collect-only -q`, focused AI/current-awareness/listener coverage, `TestNoMojibake`, and real local live-context fetches for the exact AoE4 and FFXIV screenshot prompts
