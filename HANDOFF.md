...
- Last known good verification: `.venv/bin/python -m pytest -q` locally (`344 passed`) after adding addressed-only link/image context and tightening the wake gate, plus focused regression coverage, collect-only, `TestNoMojibake`, and `git diff --check`
