...
- Status: Tinki can now be mentioned with a Discord message link to reply directly to that linked message when it is in the current server, pasted Discord message links no longer count as speaking to Tinki unless the message text contains a real Tinki mention, and OpenAI insufficient-quota failures produce a distinct out-of-money response; Instagram URL rewrites target `vxinstagram.com`; the shell deploy helper overwrites existing repo files during recursive uploads on AL2023; new AL2023 host is live, hardened, and running Python 3.11; local macOS repo has a working `.venv` pytest setup, and the repo includes low-cost monitoring setup helpers for AWS Budgets, CloudWatch alarms, and host memory/disk metrics
- Active branch: `main`
...
- Last known good verification: `python -m pytest -q` locally (`284 passed`) after tightening Tinki mention detection to actual message text, plus `python -m pytest --collect-only -q`, a focused AI listener/help slice, a mojibake scan, a temp `remote_copy` overwrite test after the deploy helper fix, and a remote post-deploy rewrite sanity check
