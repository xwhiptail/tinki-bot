...
- Last known good verification: `python -m pytest -q` locally (`304 passed`) after adding current-awareness prompt context, live source snippets, and repo permission self-healing, plus `python -m pytest --collect-only -q`, focused AI/current-awareness/runtime-bootstrap coverage, `TestNoMojibake`, and a real local live-context fetch for the AoE4 May 7 civ question
