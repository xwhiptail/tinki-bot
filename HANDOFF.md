...
- Last known good verification: `python -m pytest -q` locally (`303 passed`) after adding current-awareness prompt context and live source snippets, plus `python -m pytest --collect-only -q`, focused AI/current-awareness coverage, `TestNoMojibake`, and a real local live-context fetch for the AoE4 May 7 civ question
