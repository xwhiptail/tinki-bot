...
- Last known good verification: `python -m pytest -q` locally (`280 passed`) after adding bare links to Tinki-authored Discord messages, plus `python -m pytest --collect-only -q`, a focused AI/commands slice, a mojibake scan, a temp `remote_copy` overwrite test after the deploy helper fix, and a remote post-deploy rewrite sanity check
