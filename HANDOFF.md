...
- Last known good verification: `python -m pytest -q` locally (`295 passed`) after adding deterministic `DRG` context-switch handling and WoW/FFXIV expertise prompt coverage, plus `python -m pytest --collect-only -q`, focused AI brain/listener coverage, and `TestNoMojibake`
