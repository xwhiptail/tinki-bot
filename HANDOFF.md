...
- Status: Tinki can now be mentioned with a Discord message link to reply directly to that linked message when it is in the current server, pasted links to Tinki-authored messages can be answered without an explicit mention, and OpenAI insufficient-quota failures now produce a distinct out-of-money response instead of the generic error; Instagram URL rewrites target `vxinstagram.com`; the shell deploy helper overwrites existing repo files during recursive uploads on AL2023; new AL2023 host is live, hardened, and running Python 3.11; local macOS repo has a working `.venv` pytest setup, and the repo includes low-cost monitoring setup helpers for AWS Budgets, CloudWatch alarms, and host memory/disk metrics
- Active branch: `main`
...
- Last known good verification: `python -m pytest -q` locally (`283 passed`) after adding a distinct OpenAI out-of-money fallback, plus `python -m pytest --collect-only -q`, a focused AI listener/random-task slice, a mojibake scan, a temp `remote_copy` overwrite test after the deploy helper fix, and a remote post-deploy rewrite sanity check
