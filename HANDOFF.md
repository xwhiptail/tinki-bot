...
- Last known good verification: `.venv/bin/python -m pytest -q` locally (`330 passed`) after adding deterministic Tinki/the-bot self-status replies and the gnome-coded prompt guard, plus focused listener/prompt coverage, collect-only, `TestNoMojibake`, and `git diff --check`
