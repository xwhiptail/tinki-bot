...
- Last known good verification: `.venv/bin/python -m pytest -q` locally (`332 passed`) after hardening RoR2/DLC current-awareness and generated-reply identity cleanup, plus focused regression coverage, collect-only, `TestNoMojibake`, and `git diff --check`
