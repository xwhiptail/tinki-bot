...
- Status: Tinki can now be mentioned with a Discord message link to reply directly to that linked message when it is in the current server, pasted Discord message links and preview-surfaced mentions after a link no longer count as speaking to Tinki, erotic/spicy writing requests are deterministically deflected into a playful roast before OpenAI, calculator-vs-Final-Fantasy `DRG` context and context-switch bait are answered deterministically before OpenAI, her core prompt now makes her an expert in World of Warcraft and Final Fantasy XIV/FFXIV, AI prompts include current America/New_York and UTC date/time plus cached live source snippets for fresh/recent gaming/world questions, current-source snippets prefer official game sources when available, released/live questions are distinguished from announced/upcoming/next-news questions, and source-backed direct answers are validated before Tinki replies so stale model memory cannot override the lookup; OpenAI insufficient-quota failures produce a distinct out-of-money response; GPT-5 chat completion calls use `max_completion_tokens`; Instagram URL rewrites target `vxinstagram.com`; the shell deploy helper overwrites existing repo files during recursive uploads on AL2023; new AL2023 host is live, hardened, and running Python 3.11; local macOS repo has a working `.venv` pytest setup, and the repo includes low-cost monitoring setup helpers for AWS Budgets, CloudWatch alarms, and host memory/disk metrics
- Active branch: `main`
...
- Last known good verification: `python -m pytest -q` locally (`317 passed`) after hardening current-awareness ranking and validation for the AoE4 May 7 civ, current FFXIV expansion, and most-recent Resident Evil game questions, plus focused AI/current-awareness coverage, `TestNoMojibake`, and real local live-context fetches for the exact AoE4, FFXIV, and Resident Evil screenshot prompts
