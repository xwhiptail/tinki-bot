...
- Last known good verification: `python -m pytest -q` locally (`277 passed`) after adding linked Discord message replies, plus `python -m pytest --collect-only -q`, a focused AI/commands slice, a mojibake scan, a temp `remote_copy` overwrite test after the deploy helper fix, and a remote post-deploy rewrite sanity check
